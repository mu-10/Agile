...
const GOOGLE_MAPS_LIBRARIES = MAPS_CONFIG.libraries as unknown as (
  | "places"
  | "geometry"
)[];
...
export default function MapWeb({
  ...
  const [selectedConnectorTypes, setSelectedConnectorTypes] =
    useState<string[]>(staticConnectorTypes);
  ...
  const [autoSelectedChargingStation, setAutoSelectedChargingStation] =
    useState<any | null>(null); // Auto-selected charging station
  const [chargingStopInfo, setChargingStopInfo] = useState<any | null>(null); // Charging stop route details

  const [loadingChargingStop, setLoadingChargingStop] =
  ...
  const [needsChargingStations, setNeedsChargingStations] =
    useState<boolean>(false); // Track if we need to find charging stations
  ...
  const findChargingStop = useCallback(
    async (
      ...
      try {
        ...
        if (data.needsCharging) {
          if (data.chargingStops && data.chargingStops.length > 0) {
            ...
            console.log(
              "Multiple charging stops detected:",
              data.chargingStops.length
            );
  ...
  const calculateChargingRoute = useCallback(
    async (
      station: any,
      startCoords: google.maps.LatLngLiteral,
      endCoords: google.maps.LatLngLiteral
      ...
      try {
        ...
        if (routeToStation && routeFromStation) {
          ...
          const chargingTime =
            (station.estimatedChargingTimeMinutes || 30) * 60; // convert to seconds

          const totalTravelTime = Math.round(
            (timeToStation + timeFromStation + chargingTime) / 60
          ); // in minutes
          const originalTravelTime = duration
            ? parseInt(duration.replace(/\D/g, ""))
            : Math.round((timeToStation + timeFromStation) / 60);

          const routeDetails = {
            totalTravelTime,
            originalTravelTime,
            totalDistanceViaStation:
              station.totalDistanceViaStation ||
              ((legToStation.distance?.value || 0) +
                (legFromStation.distance?.value || 0)) /
                1000,
            ...
            remainingRangeAtDestination:
              station.remainingRangeAtDestination || "Unknown",
          };
  ...
  const calculateMultiStopRoute = useCallback(
    async (
      ...
      try {
        console.log(
          `Calculating route through ${chargingStops.length} charging stops`
        );
        ...
        const firstStopRoute = await new Promise<google.maps.DirectionsResult>(
          (resolve, reject) => {
            directionsServiceRef.current!.route(
              {
                origin: startCoords,
                destination: {
                  lat: chargingStops[0].latitude,
                  lng: chargingStops[0].longitude,
                },
                travelMode: google.maps.TravelMode.DRIVING,
              },
              (result, status) => {
                if (status === "OK" && result) {
                  ...
                  reject(
                    new Error(`Route to first charging stop failed: ${status}`)
                  );
                }
              }
            );
        ...
        for (let i = 0; i < chargingStops.length - 1; i++) {
          const segmentRoute = await new Promise<google.maps.DirectionsResult>(
            (resolve, reject) => {
              directionsServiceRef.current!.route(
                {
                  origin: {
                    lat: chargingStops[i].latitude,
                    lng: chargingStops[i].longitude,
                  },
                  destination: {
                    lat: chargingStops[i + 1].latitude,
                    lng: chargingStops[i + 1].longitude,
                  },
                  travelMode: google.maps.TravelMode.DRIVING,
                },
                (result, status) => {
                  if (status === "OK" && result) {
                    ...
                    reject(
                      new Error(
                        `Route between charging stops ${i} and ${
                          i + 1
                        } failed: ${status}`
                      )
                    );
        ...
        const lastStopRoute = await new Promise<google.maps.DirectionsResult>(
          (resolve, reject) => {
            directionsServiceRef.current!.route(
              {
                origin: {
                  lat: chargingStops[chargingStops.length - 1].latitude,
                  lng: chargingStops[chargingStops.length - 1].longitude,
                },
                destination: endCoords,
                travelMode: google.maps.TravelMode.DRIVING,
              },
              (result, status) => {
                if (status === "OK" && result) {
                  ...
                  reject(
                    new Error(`Route from last charging stop failed: ${status}`)
                  );
                }
              }
            );
        ...
        console.log(
          `First segment: Start → ${chargingStops[0].title || "Stop 1"}`
        );
        console.log(
          `Last segment: ${
            chargingStops[chargingStops.length - 1].title || "Last Stop"
          } → End`
        );
        ...
        if (routeSegments.length >= 3) {
          ...
          console.log(
            `Setting ${intermediateSegments.length} intermediate segments for orange rendering`
          );
          intermediateSegments.forEach((segment, index) => {
            console.log(
              `  Intermediate segment ${index + 1}: ${
                chargingStops[index].title || `Stop ${index + 1}`
              } → ${chargingStops[index + 1].title || `Stop ${index + 2}`}`
            );
          });
        ...
        console.log(
          `Total route: ${(totalDist / 1000).toFixed(
            1
          )}km, ${totalTravelTime}min (including ${
  ...
  const [stationReachability, setStationReachability] = useState<{
    [key: string]: { reachable: boolean; message: string };
  }>({});
  ...
  const checkStationReachability = useCallback(
    async (station: any, batteryRange: number) => {
      ...
      try {
        ...
        if (!startCoords) {
          return {
            reachable: false,
            message: "Could not determine starting coordinates",
          };
        }

        // Call backend to validate reachability with actual route calculation
        const response = await fetch(
          API_ENDPOINTS.validateStationReachability(),
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          const result = {
            reachable: true,
            message:
              "Backend validation unavailable - cannot verify reachability",
          };
        ...
        const result = {
          reachable: true,
          message:
            "Unable to validate reachability - backend service unavailable",
        };
  ...
  useEffect(() => {
    if (
      ...
      batteryRange &&
      showChargingRoute &&
      chargingStopInfo?.needsCharging
    ) {
  ...
  const filteredStations = useMemo(() => {
    ...
    let filtered = stations.filter(
      (station) =>
        station.connections &&
        station.connections.some((conn: any) =>
          selectedConnectorTypes.includes(conn.type)
        )
    );
  ...
  const onLoad = useCallback(
    (mapInstance: google.maps.Map) => {
      ...
      const chargerInfoContent = `<div style="padding:10px;min-width:160px;background:${
        mapStyle ? "#27272a" : "#fff"
      };color:${
        mapStyle ? "#d1d5db" : "#111827"
      };border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,0.15);font-family:system-ui,sans-serif;">
      ...
      async (result, status) => {
        if (status === "OK" && result) {
          ...
          const routeDistance = parseFloat(
            leg.distance?.text?.replace(/[^\d.]/g, "") || "0"
          );
          ...
          if (routeDistance > batteryRange && batteryRange > 0) {
            ...
            let startCoords =
              parseLatLng(start) ||
              (typeof origin === "object" && "lat" in origin ? origin : null);
            let endCoords =
              parseLatLng(end) ||
              (typeof destination === "object" && "lat" in destination
                ? destination
                : null);
  ...
  const [recommendedStation, setRecommendedStation] = useState<any | null>(
    null
  );
  ...
  const handleRecommendedStationClick = useCallback(
    (station: any) => {
      if (map && station) {
    ...
    <div style={{ width: "100%", height: "100vh", position: "relative" }}>
      ...
      <div style={{ position: "absolute", top: 20, right: 20, zIndex: 1001 }}>
        ...
        >
          ...
          <span style={{ float: "right", fontSize: 14, color: "#333" }}>
      ...
      >
        ...
        {directionsResponse && (
          <DirectionsRenderer
            ...
            options={{
              ...
              polylineOptions: {
                strokeColor:
                  showChargingPanels && allChargingStops.length > 1
                    ? "#4285F4"
                    : showChargingPanels && allChargingStops.length === 1
                    ? "#FF6B35"
                    : "#4285F4", // Blue for first segment to charging stop, orange for single stop, blue for normal routes
                strokeWeight: 6,
                strokeOpacity: 0.8,
              },
              markerOptions: {
            ...
            options={{
              ...
              polylineOptions: {
                strokeColor:
                  showChargingPanels && allChargingStops.length > 1
                    ? "#FF6B35"
                    : "#87CEEB", // Orange for intermediate charging segments, light blue for regular alternatives
                strokeWeight:
                  showChargingPanels && allChargingStops.length > 1 ? 6 : 4,
                strokeOpacity:
                  showChargingPanels && allChargingStops.length > 1 ? 0.8 : 0.6,
              },
              markerOptions: {
                icon: {
                  path: google.maps.SymbolPath.CIRCLE,
                  scale:
                    showChargingPanels && allChargingStops.length > 1 ? 8 : 6,
                  fillColor:
                    showChargingPanels && allChargingStops.length > 1
                      ? "#FF6B35"
                      : "#87CEEB",
                  fillOpacity:
                    showChargingPanels && allChargingStops.length > 1 ? 1 : 0.8,
                  strokeColor: "#FFFFFF",
                  strokeWeight:
                    showChargingPanels && allChargingStops.length > 1 ? 2 : 1,
                },
              },
            }}
          />
        ...
        {(() => {
          ...
          if (
            !Array.isArray(stationsToRender) ||
          ...
          return stationsToRender
            .map((station: any) => {
              ...
              if (
                autoSelectedChargingStation &&
                station.id === autoSelectedChargingStation.id
              ) {
                return null;
              }
        ...
        {showRecommendedLocations &&
          ...
          !filteredStations.some(
            (station) => station.id === recommendedStation.id
          ) && (
            <Marker
              ...
              onClick={() => {
                console.log(
                  "Recommended station clicked:",
                  recommendedStation.title
                );
        ...
        <StationInfoWindow
          key={
            selectedStation
              ? `station-${selectedStation.id || selectedStation.title}`
              : "no-station"
          }
          ...
          isRecommended={
            selectedStation &&
            autoSelectedChargingStation &&
            selectedStation.id === autoSelectedChargingStation.id
          }
          isDarkMode={!!mapStyle}
        />
      </GoogleMap>
      ...
      {backendError && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div
              style={{ fontWeight: "500", color: "#333", marginBottom: "2px" }}
            >
              Backend Offline
            </div>
